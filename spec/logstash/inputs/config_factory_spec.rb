...
require_relative '../../../lib/logstash/inputs/delete_config'
...
describe ConfigFactory do
  ...
  let(:delete_config) do
    base_config.merge({ :document_ids => %w[foo bar], :sync_type => 'delete' })
  end
  ...
  it 'creates the right config' do
    ...
    expect(ConfigFactory.get(delete_config)).to be_instance_of(DeleteConfig)
  end
